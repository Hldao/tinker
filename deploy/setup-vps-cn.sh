...
echo "     值: $IP"
echo ""
echo "  ${YELLOW}3.${NC} 验证 (按生效顺序):"
echo "     a) 公网 IP: 浏览器打开 http://$IP:$PORT/api/health"
echo "        看到 {\"status\":\"ok\"...} → 安全组 OK"
echo "     b) 域名 (DNS 生效后): 浏览器打开 http://你的域名:$PORT"
echo "        看到 Tinker 主页 → 全部完成 🎉"
echo "        DNS 是否生效可用: dig +short 你的域名 (本地终端)"
echo ""
echo -e "${BOLD}日常运维:${NC}"
echo "  看日志:   cd ~/tinker && $SUDO docker compose logs -f tinker"
echo "  更新:     cd ~/tinker && $SUDO git pull && $SUDO docker compose up -d --build"
echo "  重启:     cd ~/tinker && $SUDO docker compose restart"
echo ""
echo -e "${BOLD}备案后切换到标准 80/443 + HTTPS:${NC}"
echo "  cd ~/tinker"
echo "  bash deploy/setup-vps.sh 你的域名 your@email.com"
echo ""
echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
