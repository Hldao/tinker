...
const STOP_WORDS = new Set([
  ...
  '我们', '你们', '他们', '她们', '它们', '咱们', '大家', '自己', '哪个', '哪种',
  '我的', '你的', '他的', '她的', '我们的', '你们的', '他们的',
  '们的', '们是', '们都', '们也', '们就',  // 代词 + 助词的常见 2 字子串
  '怎么', '什么', '为什么', '一下', '一次', '一直', '一遍', '这个', '那个', '不是',
  '没有', '可以', '应该', '需要', '已经', '正在', '又是', '都是', '只是', '感觉',
  '试试', '继续', '不知', '知道', '现在', '今天', '昨天',
  '一个', '一种', '一些', '一定', '一样', '一点', '一条', '一段', '一次性', '不同',
  '相同', '比较', '比如', '大概', '大约', '差不多', '可能', '或许', '哪里', '哪些',
  '么用', '么搞', '么写', '么配', '么调', '么做', '么开', '么开始',
  '开始', '配置', '设置', '使用', '解决', '处理', '调用', '运行', '安装',
...
// 代词类前缀 · 滑动窗口可能切出 "我们X" 这种 · 全部当 noise 过滤
// (替代之前要排除整个 token · 现在前缀派生也排除)
const PRONOUN_PREFIXES = ['我们', '你们', '他们', '她们', '咱们', '我的', '你的', '他的', '我', '你', '他', '她'];
...
const NOISE_PREFIXES = ['还是', '又是', '怎么', '又试', '又报', '又错', '又挂', '没生', '没反', '为什', '不知', '搞定', '原来', '终于', '可以', '生效', '搞懂', '学会', '看懂', '上手', '么开', '么用', '么搞', '么写', '么配', '么调', '么做'];
const NOISE_SUFFIXES = ['不行', '不通', '失败', '报错', '没用', '没生效', '没反应', '搞砸', '挂了', '崩了', '懂了', '会了', '开始', '配置', '设置', '使用'];
...
function isSignalNoise(t) {
  ...
  // 代词前缀派生 · 比如 "我们class"  "我的项目"  滑窗会切出 "我们cl" 这种 (跨中英也算)
