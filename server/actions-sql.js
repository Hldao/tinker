...
// ============================================
// 团队待办 (v1.x) · 工作室维度简单 todolist
// 工作日志复用 updates (记一笔) · 这里只管待办
// ============================================
function isStudioMember(studioId, userId) {
  return !!db.prepare('SELECT 1 FROM studio_members WHERE studio_id = ? AND user_id = ?').get(studioId, userId);
}

function addTodo({ studioId, text, due, ownerHandle }, { currentUserId }) {
  if (!studioId) throw new Error('缺少工作室');
  if (!text || !text.trim()) throw new Error('待办是空的');
  if (!isStudioMember(studioId, currentUserId)) throw new Error('你不在这个团队里');
  let ownerId = currentUserId;
  if (ownerHandle) {
    const uid = userIdFromHandle(ownerHandle);
    if (uid && isStudioMember(studioId, uid)) ownerId = uid;
  }
  const id = db.uuidv7();
  const dueVal = (due && /^\d{4}-\d{2}-\d{2}$/.test(due)) ? due : null;
  db.prepare(`
    INSERT INTO todos (id, studio_id, owner_id, creator_id, text, due, done, created_at)
    VALUES (?, ?, ?, ?, ?, ?, 0, ?)
  `).run(id, studioId, ownerId, currentUserId, text.trim(), dueVal, Date.now());
  return { ok: true, id };
}

function toggleTodo({ todoId, done }, { currentUserId }) {
  const t = db.prepare('SELECT studio_id, done FROM todos WHERE id = ?').get(todoId);
  if (!t) throw new Error('找不到这条待办');
  if (!isStudioMember(t.studio_id, currentUserId)) throw new Error('你不在这个团队里');
  const next = (typeof done === 'boolean') ? (done ? 1 : 0) : (t.done ? 0 : 1);
  db.prepare('UPDATE todos SET done = ?, done_at = ? WHERE id = ?')
    .run(next, next ? Date.now() : null, todoId);
  return { ok: true, done: !!next };
}

function deleteTodo({ todoId }, { currentUserId }) {
  const t = db.prepare('SELECT studio_id FROM todos WHERE id = ?').get(todoId);
  if (!t) throw new Error('找不到这条待办');
  if (!isStudioMember(t.studio_id, currentUserId)) throw new Error('你不在这个团队里');
