// 一次性脚本: 把现有 data.json 灌进 SQLite (Phase C 切换前必跑一次)
//
// 用法:
//   node migrate-from-json.js [data.json 路径] [--force]
//   默认路径: ./data.json
//   --force: 即使 DB 非空也清空 + 重建 (危险)
//
// 安全:
...
if (!fs.existsSync(jsonPath)) {
  console.error('✗ 找不到 data.json:', jsonPath);
  process.exit(1);
}
...
if (userCount > 0 && !force) {
  console.error('✗ DB 已有 ' + userCount + ' 个用户 · 拒绝迁移 (避免覆盖)');
  console.error('  传 --force 清空 + 重建 (危险 · 现有数据会丢)');
  process.exit(1);
}

console.log('━━━ Tinker data.json → SQLite ━━━');
console.log('  source:', jsonPath);
console.log('  目标 DB:', process.env.DB_FILE || 'server/tinker.db');

const handleToId = {};

const txn = db.transaction(() => {
  if (force) {
    console.log('  --force · 清空所有表...');
  ...
  const insTool = db.prepare(`
    INSERT INTO project_tools (project_id, tool) VALUES (?, ?)
  `);
  for (const p of (data.projects || [])) {
    const ownerId = handleToId[p.owner];
    if (!ownerId) {
      console.warn('  ⚠ project ' + p.id + ' owner @' + p.owner + ' 不在 users · 跳过');
      continue;
    }
    insProject.run(
    ...
    // updates + images + usedBy
    const insUpdate = db.prepare(`
      INSERT INTO updates (id, project_id, text, prompt, at) VALUES (?, ?, ?, ?, ?)
    `);
    const insImg = db.prepare(`
      INSERT INTO images (id, src, caption, created_at) VALUES (?, ?, ?, ?)
    `);
    const insUpImg = db.prepare(`
      INSERT INTO update_images (update_id, image_id, position) VALUES (?, ?, ?)
    `);
    const insMu = db.prepare(`
    ...
    `);
    const insNoteImg = db.prepare(`
      INSERT INTO note_images (note_id, image_id, position) VALUES (?, ?, ?)
    `);
    ...
    // reactions
    const insReact = db.prepare(`
      INSERT INTO reactions (project_id, user_id, type, at) VALUES (?, ?, ?, ?)
    `);
    const insTinkered = db.prepare(`
      INSERT INTO tinkered (id, parent_project_id, user_id, name, link, at) VALUES (?, ?, ?, ?, ?, ?)
    `);
  ...
  for (const n of (data.notifications || [])) {
    ...
    if (!targetId || !fromId) {
      console.warn('  ⚠ notification 跳过 (user 不存在):', n);
      continue;
    }
    insNotif.run(
  ...
  console.log('  · ' + (data.notifications || []).length + ' 条');

  // 4. starters
  console.log('━ starters');
  const insStarter = db.prepare(`
    INSERT INTO starters (title, prompt, tool_name, tool_url, position) VALUES (?, ?, ?, ?, ?)
  `);
  (data.starters || []).forEach((s, idx) => {
    insStarter.run(s.title, s.prompt, s.toolName, s.toolUrl, idx);
  });
  console.log('  · ' + (data.starters || []).length + ' 条');
  ...
  console.log('  · ' + (data.availableTools || []).length + ' 个工具');
});
...
try {
  ...
  console.log('  updates:', db.prepare('SELECT COUNT(*) AS c FROM updates').get().c);
  console.log('  notes:', db.prepare('SELECT COUNT(*) AS c FROM notes').get().c);
  console.log('  reactions:', db.prepare('SELECT COUNT(*) AS c FROM reactions').get().c);
  console.log('  tinkered:', db.prepare('SELECT COUNT(*) AS c FROM tinkered').get().c);
