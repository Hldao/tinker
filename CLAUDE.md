...
| 说"帮我发个求方法 / 我缺个 X 的方法 / 求人指路" | `tinker seek -m "缺什么方法"` 发一条求方法 (进方法库「有人在找」· 注意:不是 ship 的求反馈) |
| 说"把这个沉淀成方法 / 存成方法" (做通了一件别人能复用的事) | `tinker contribute [updateId]` 把某条 update 标成方法 (进方法库「沉淀的方法」) |
...
- **voice fingerprint**:`.tinker/voice-fingerprint.md` 有作者真实风格画像 · 起草前读它。**但它是从过去样本自动生成的 · 会滞后于作者真实风格 · 甚至跟上面的明确规则冲突**(踩过坑:画像旧版说"中点 · 是招牌 / 极高",直接跟"别堆中点"打架,导致起草时堆了一堆 · 被用户抓)。**冲突时一律以上面的明确规则为准 · 别照画像。** 画像该定期重新生成(`tinker voice analyze`)跟上真实风格。
...
- `cli/bin/tinker.js` — CLI 主文件 (单文件 11000+ 行)
