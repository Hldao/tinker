...
- 短句优先 · 一条进展 200-400 字 · 不需要排版
- 不写"今天 / 最近"开头(平台已经显示时间)
- 支持 inline markdown:**粗体** 和 `代码片段` · **不要** 用 # 标题 / - 列表 / 引用块这种块级元素
- 反"看着像产品宣传"·反"流量诱导"·反"AI 装大佬"
...
2. **看未 commit 的改动**:`git diff --shortstat`
3. **起草 1-3 条候选进展**(Tinker voice · 每条 200-400 字)
4. **给我看候选 · 每条附"自评"**(一句话说为什么这条值得发) · 让我挑
...
- **永远别替我决定"该不该发"** · 只能起草 + 等我确认 · 半自动是 Tinker 的设计
- **不要凑数** —— git 历史全是 typo / 格式调整时 · 直接说"这段没什么值得发的" · 别硬写
